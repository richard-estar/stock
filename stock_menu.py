# -*- coding: utf-8 -*-
"""
Created on Tue Oct 19 21:23:55 2021
...
def obj_encoder(obj):
        ...
        for o in obj.DataList:
            d = data_encoder(o)
            dlist.append(d)
        stock_dict = dict(symbol=obj.symbol, name = obj.name, shares = obj.shares, 
DataList = dlist)
        return stock_dict
def obj_decoder(obj):
        symbol = obj["symbol"]
...
def file_processing(stock_list):
    ...
    while choice != "E":
        choice = input(" To save data press (S), To load data press (L), To import data press (D) Or to exit press (E)").upper()
        ...
        if choice == "S":
            ...
            try:
                with open("stock_data.json","r") as f:
                    ...
                    print("file loaded from stock_data.json")
            except IOError:
                print("File not created")
            break
